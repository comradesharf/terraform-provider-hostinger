...
4. In `List`, read configuration with `req.Config`, iterate pages, log the page, check transport/status/nil-body/nil-data failures, merge each item into the singular resource model, set display name and identity, and set the resource only when `req.IncludeResource` is true.
