...
- `Read`: load configuration into the model with `req.Config`, reject unknown optional filter values before the API call, build any params, call the `WithResponse` method, check `StatusCode() != http.StatusOK`, reject a nil `JSON200`, map the response into model structs, and persist with `resp.State.Set`
