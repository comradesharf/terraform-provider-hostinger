...
var (
	_ action.Action              = &VPSFirewallActivateAction{}
	_ action.ActionWithConfigure = &VPSFirewallActivateAction{}
)

func NewVPSFirewallActivateAction() action.Action {
	return &VPSFirewallActivateAction{}
}

type VPSFirewallActivateAction struct {
	client *client.ClientWithResponses
}

type VPSFirewallActivateActionModel struct {
...
func (a *VPSFirewallActivateAction) Configure(ctx context.Context, req action.ConfigureRequest, resp *action.ConfigureResponse) {
...
func (a *VPSFirewallActivateAction) Schema(ctx context.Context, req action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Activates a VPS firewall.",
		Attributes: map[string]schema.Attribute{
			"firewall_id": schema.Int64Attribute{
				Required:            true,
				MarkdownDescription: "The ID of the firewall to activate.",
			},
			"virtual_machine_id": schema.Int64Attribute{
				Required:            true,
				MarkdownDescription: "The ID of the virtual machine to associate with the firewall.",
			},
...
func (a *VPSFirewallActivateAction) Metadata(ctx context.Context, req action.MetadataRequest, resp *action.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_vps_firewall_activate"
}

func (a *VPSFirewallActivateAction) Invoke(ctx context.Context, req action.InvokeRequest, resp *action.InvokeResponse) {
	var config VPSFirewallActivateActionModel
	...
	if config.VirtualMachineID.IsNull() || config.VirtualMachineID.IsUnknown() {
		resp.Diagnostics.AddError(
			"Invalid Virtual Machine ID",
			"The virtual_machine_id attribute is required and must be a valid integer.",
		)
	}
	...
	response, err := a.client.VPSActivateFirewallV1WithResponse(
		ctx,
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Unable to Activate VPS Firewall",
