...
func TestAccVPSFirewallActivateAction(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
			testAccVPSFirewallActivateActionPreCheck(t)
		},
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVPSFirewallActivateActionConfig,
				PostApplyFunc: func() {
					client := newMockServerClient()

					err := client.VerifySequence(
						mockserver.Request().Path("/api/vps/v1/firewall/65224/activate/12345").Method("POST"),
...
func testAccVPSFirewallActivateActionPreCheck(t *testing.T) {
	client := newMockServerClient()
...
const testAccVPSFirewallActivateActionConfig = `
resource "terraform_data" "test" {
	lifecycle {
		action_trigger {
			events = [after_create]
			actions = [action.hostinger_vps_firewall_activate.test]
		}
	}
}

action "hostinger_vps_firewall_activate" "test" {
	config {
		firewall_id = 65224
