...
func TestAccVPSFirewallSyncAction(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
			testAccVPSFirewallSyncActionPreCheck(t)
		},
		...
		Steps: []resource.TestStep{{
			Config: testAccVPSFirewallSyncActionConfig,
			PostApplyFunc: func() {
				client := newMockServerClient()

				err := client.VerifySequence(
					mockserver.Request().Path("/api/vps/v1/firewall/65224/sync/12345").Method("POST"),
...
func testAccVPSFirewallSyncActionPreCheck(t *testing.T) {
	client := newMockServerClient()
...
const testAccVPSFirewallSyncActionConfig = `
resource "terraform_data" "test" {
	lifecycle {
		action_trigger {
			events = [after_create]
			actions = [action.hostinger_vps_firewall_sync.test]
		}
	}
}

action "hostinger_vps_firewall_sync" "test" {
	config {
		firewall_id = 65224
