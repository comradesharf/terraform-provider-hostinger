...
func TestAccVPSVirtualMachineRecreateAction(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
			testAccVPSVirtualMachineRecreateActionPreCheck(t)
		},
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVPSVirtualMachineRecreateActionConfig,
				PostApplyFunc: func() {
					client := newMockServerClient()
					if err := client.VerifySequence(
						mockserver.Request().Path("/api/vps/v1/virtual-machines/12345/recreate").Method("POST"),
...
func testAccVPSVirtualMachineRecreateActionPreCheck(t *testing.T) {
	client := newMockServerClient()
...
const testAccVPSVirtualMachineRecreateActionConfig = `
resource "terraform_data" "test" {
	lifecycle {
		action_trigger {
			events = [after_create]
			actions = [action.hostinger_vps_virtual_machine_recreate.test]
		}
	}
}

action "hostinger_vps_virtual_machine_recreate" "test" {
	config {
