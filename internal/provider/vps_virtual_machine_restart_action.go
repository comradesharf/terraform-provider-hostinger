...
var (
	_ action.Action              = &VPSVirtualMachineRestartAction{}
	_ action.ActionWithConfigure = &VPSVirtualMachineRestartAction{}
)

func NewVPSVirtualMachineRestartAction() action.Action {
	return &VPSVirtualMachineRestartAction{}
}

type VPSVirtualMachineRestartAction struct {
	client *client.ClientWithResponses
}

type VPSVirtualMachineRestartActionModel struct {
	Timeouts         timeouts.Value `tfsdk:"timeouts"`
	VirtualMachineID types.Int64    `tfsdk:"virtual_machine_id"`
	WaitForAction    types.Bool     `tfsdk:"wait_for_action"`
}

func (a *VPSVirtualMachineRestartAction) Configure(ctx context.Context, req action.ConfigureRequest, resp *action.ConfigureResponse) {
...
func (a *VPSVirtualMachineRestartAction) Schema(ctx context.Context, req action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Restarts a VPS virtual machine.",
		Attributes: map[string]schema.Attribute{
			"virtual_machine_id": schema.Int64Attribute{
				Required:            true,
				MarkdownDescription: "The ID of the virtual machine to restart.",
			},
...
func (a *VPSVirtualMachineRestartAction) Metadata(ctx context.Context, req action.MetadataRequest, resp *action.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_vps_virtual_machine_restart"
}

func (a *VPSVirtualMachineRestartAction) Invoke(ctx context.Context, req action.InvokeRequest, resp *action.InvokeResponse) {
	var config VPSVirtualMachineRestartActionModel
	...
	response, err := a.client.VPSRestartVirtualMachineV1WithResponse(
		ctx,
		client.VirtualMachineId(config.VirtualMachineID.ValueInt64()),
	)
	if err != nil {
		resp.Diagnostics.AddError("Unable to Restart VPS Virtual Machine", fmt.Sprintf("Got error: %s", err))
		return
	}
	if response.StatusCode() != http.StatusOK {
		resp.Diagnostics.AddError("Unable to Restart VPS Virtual Machine", fmt.Sprintf("Unexpected status code: %d, response: %s", response.StatusCode(), string(response.Body)))
		return
	}
	if response.JSON200 == nil || response.JSON200.Id == nil {
		resp.Diagnostics.AddError("Unable to Restart VPS Virtual Machine", "Response body is nil or missing ID")
		return
	}
	...
	poll:
		for {
			...
			case client.VPSV1ActionActionResourceStateError:
				resp.Diagnostics.AddError("Unable to Restart VPS Virtual Machine", "The virtual machine restart action failed")
				return
			default:
				select {
				case <-ctx.Done():
					resp.Diagnostics.AddError("Unable to Restart VPS Virtual Machine", "The virtual machine restart action timed out")
					return
