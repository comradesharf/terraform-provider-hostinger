...
func TestAccVPSVirtualMachineStopAction(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
			testAccVPSVirtualMachineStopActionPreCheck(t)
		},
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVPSVirtualMachineStopActionConfig,
				PostApplyFunc: func() {
					client := newMockServerClient()

					err := client.VerifySequence(
						mockserver.Request().Path("/api/vps/v1/virtual-machines/12345/stop").Method("POST"),
...
func testAccVPSVirtualMachineStopActionPreCheck(t *testing.T) {
	client := newMockServerClient()
...
const testAccVPSVirtualMachineStopActionConfig = `
resource "terraform_data" "test" {
	lifecycle {
		action_trigger {
			events = [after_create]
			actions = [action.hostinger_vps_virtual_machine_stop.test]
		}
	}
}

action "hostinger_vps_virtual_machine_stop" "test" {
	config {
